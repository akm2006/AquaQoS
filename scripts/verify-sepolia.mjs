...
const declarations = {
  AquaQoSRouter: 'contracts/AquaQoSRouter.sol:AquaQoSRouter',
  AquaQoSVault: 'contracts/AquaQoSVault.sol:AquaQoSVault',
  TokenMock: '@1inch/solidity-utils/contracts/mocks/TokenMock.sol:TokenMock',
};
report.verification = [];
for (const deployed of report.deployments) {
  ...
  const response = await fetch(`https://sourcify.dev/server/v2/verify/${report.chainId}/${deployed.address}`, {
    method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({
      ...
      contractIdentifier: declarations[deployed.name], creationTransactionHash: deployed.transactionHash,
