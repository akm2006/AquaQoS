# AquaQoS documentation

Start with the [project charter](PROJECT_CHARTER.md), then follow the shortest path for
your purpose. Public claims should link to source, tests, retained evidence, or a primary
source recorded here.

## Protocol

- [CAPACITY_GUARD specification](CAPACITY_GUARD_SPEC.md)
- [Problem reproduction](PROBLEM_REPRODUCTION.md)
- [Protocol baseline](PROTOCOL_BASELINE.md)
- [Architecture decisions](DECISIONS.md)
- [Threat model](THREAT_MODEL.md)

## Verification and deployments

- [Release verification](RELEASE_VERIFICATION.md)
- [Sepolia deployment](SEPOLIA_DEPLOYMENT.md)
- [Authenticated Ethereum-fork proof](FORK_PROOF.md)
- [Transaction validation](TRANSACTION_VALIDATION.md)

## Benchmarks

- [Methodology](BENCHMARK_METHODOLOGY.md)
- [Results](BENCHMARK_RESULTS.md)
- [Rejection replay](REJECTION_REPLAY.md)
- [Lifecycle gas](LIFECYCLE_GAS.md)

## Security and limitations

- [Current Sepolia review](SECURITY_REVIEW_SEPOLIA.md)
- [Release review](SECURITY_REVIEW_RELEASE.md)
- [Implementation review](SECURITY_REVIEW_V0.md)
- [Acceptance criteria](ACCEPTANCE_CRITERIA.md)

These are internal reviews and executable checks, not an external audit or production
certification.

## Product

- [Design system](product/DESIGN_SYSTEM.md)
- [Application architecture](product/APP_ARCHITECTURE.md)
- [Local workspace guide](product/DEMO.md)

## Project record

- [Current status](STATUS.md)
- [Execution plan](EXECUTION_PLAN.md)
- [Primary research sources](RESEARCH_SOURCES.md)
- [Third-party provenance](THIRD_PARTY.md)
- [Historical development record](archive/development/STATUS_HISTORY.md)
- [ETHOnline 2026 provenance and requirements](archive/ethonline-2026/)

Raw machine-readable benchmark and deployment evidence remains in `benchmarks/raw/` and
`deployments/`; it is intentionally retained outside the narrative documentation tree.
